...
S02: очередь TreeWalker и addedNodes-итераторы обрабатываются порциями; Set по ссылке
на video исключает повторы slot/light DOM и допускает повторное присоединение того же узла
во время поиска. Native getter вызывается только для допустимых HTML author-shadow hosts,
поэтому UA roots video/input не обходятся. Кандидаты возвращаются только после полного
обхода текущей очереди и свежего IO-ответа. Если подходящих video нет, окно остаётся
активным до deadline; observers и два host-pass принадлежат только этому окну.

`complete` означает завершённый обход доступных roots в пределах бюджета; `closedRoots`
отдельно сообщает capability. При отсутствии native accessor открытые roots продолжают
работать, а ON содержит `coverageWarning: OPEN_ROOTS_ONLY`. Пустой результат при такой
capability не выдаётся за доказанное отсутствие video. Budget/IO failure всегда возвращает
`INCOMPLETE_COVERAGE` и запрещает выбор победителя.

С S03 coordinator распределяет окно 3s между документами: до трёх фаз сбора около
0/500/1500 ms, каждая с оставшимся deadline. Локальные visits суммируются между фазами,
поэтому повторный поиск не обнуляет лимит document/tab. Пустой родитель с iframe может
закончить локальный scan раньше, передав iframe references для bind; новые frames допускаются
только пока текущая попытка активна. Перед запуском scan резервируется element allowance;
сумма уже потраченного и зарезервированного не превышает 100k.
